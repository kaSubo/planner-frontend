...
export const filterTasks = (tasks: ITaskResponse[] | undefined, value: string) => {
	switch (value) {
		case 'today':
			return tasks?.filter(
				(item) => dayjs(item.createdAt).isSame(FILTERS.today, 'day') && !item.isCompleted
			);

		case 'tomorrow':
			return tasks?.filter(
				(item) =>
					dayjs(item.createdAt).isSame(FILTERS.tomorrow, 'day') && !item.isCompleted
			);
		...
		case 'later':
			return tasks?.filter(
				(item) =>
					(dayjs(item.createdAt).isAfter(FILTERS['on-next-week']) || !item.createdAt) &&
					!item.isCompleted
			);
